window.questionsByPrize = {    500: [
        {
            question: "Która planeta jest najbliżej Słońca?",
            answers: ["Wenus", "Mars", "Merkury", "Ziemia"],
            correct: 2
        },
        {
            question: "Ile nóg ma pająk?",
            answers: ["6", "8", "10", "12"],
            correct: 1
        },
        {
            question: "Jaki kolor powstaje po zmieszaniu czerwonego i żółtego?",
            answers: ["Zielony", "Pomarańczowy", "Fioletowy", "Niebieski"],
            correct: 1
        },
        {
            question: "Który z tych zwierząt to ssak?",
            answers: ["Rekin", "Delfin", "Orzeł", "Wąż"],
            correct: 1
        },
        {
            question: "Które z tych miast leży nad Wisłą?",
            answers: ["Poznań", "Warszawa", "Wrocław", "Szczecin"],
            correct: 1
        },
        {
            question: "Jak nazywa się największy polski port morski?",
            answers: ["Gdynia", "Gdańsk", "Szczecin", "Świnoujście"],
            correct: 1
        },
        {
            question: "Który z tych krajów leży w Afryce?",
            answers: ["Brazylia", "Egipt", "Hiszpania", "Indie"],
            correct: 1
        },
        {
            question: "Jak nazywa się największy ocean na świecie?",
            answers: ["Atlantycki", "Spokojny", "Indyjski", "Arktyczny"],
            correct: 1
        },
        {
            question: "Który z tych języków jest językiem urzędowym w Szwajcarii?",
            answers: ["Włoski", "Hiszpański", "Rosyjski", "Polski"],
            correct: 0
        },
        {
            question: "Jak nazywa się największy ssak na świecie?",
            answers: ["Słoń afrykański", "Płetwal błękitny", "Hipopotam", "Nosorożec"],
            correct: 1
        },
    ],    1000: [
        {
            question: "Kto napisał 'Pan Tadeusz'?",
            answers: ["Juliusz Słowacki", "Adam Mickiewicz", "Henryk Sienkiewicz", "Cyprian Kamil Norwid"],
            correct: 1
        },
        {
            question: "Stolica Francji to:",
            answers: ["Lyon", "Marsylia", "Paryż", "Nice"],
            correct: 2
        },
        {
            question: "Ile kontynentów jest na Ziemi?",
            answers: ["5", "6", "7", "8"],
            correct: 2
        },
        {
            question: "Który ocean jest największy?",
            answers: ["Atlantycki", "Indyjski", "Arktyczny", "Spokojny"],
            correct: 3
        },
        {
            question: "Najwyższa góra świata to:",
            answers: ["K2", "Mont Blanc", "Mount Everest", "Kilimandżaro"],
            correct: 2
        },
        {
            question: "W którym kraju leży Rzym?",
            answers: ["Hiszpania", "Francja", "Grecja", "Włochy"],
            correct: 3
        },
        {
            question: "Który z tych krajów nie graniczy z Polską?",
            answers: ["Czechy", "Słowacja", "Węgry", "Niemcy"],
            correct: 2
        },
        {
            question: "Jak nazywa się najwyższy szczyt w Polsce?",
            answers: ["Rysy", "Giewont", "Babia Góra", "Śnieżka"],
            correct: 0
        },
        {
            question: "Który z tych pisarzy był laureatem Nagrody Nobla?",
            answers: ["Adam Mickiewicz", "Henryk Sienkiewicz", "Juliusz Słowacki", "Bolesław Prus"],
            correct: 1
        },
        {
            question: "Jak nazywa się największe jezioro w Polsce?",
            answers: ["Śniardwy", "Mamry", "Hańcza", "Miedwie"],
            correct: 0
        },
        {
            question: "Który z tych krajów nie należy do Unii Europejskiej?",
            answers: ["Polska", "Norwegia", "Czechy", "Włochy"],
            correct: 1
        },
        {
            question: "Jak nazywa się najdłuższa rzeka Europy?",
            answers: ["Dunaj", "Wołga", "Ren", "Loara"],
            correct: 1
        },
        {
            question: "Który z tych filmów wyreżyserował Steven Spielberg?",
            answers: ["Titanic", "Jurassic Park", "Avatar", "Gladiator"],
            correct: 1
        },
        {
            question: "Jak nazywa się największa wyspa na świecie?",
            answers: ["Grenlandia", "Madagaskar", "Borneo", "Sumatra"],
            correct: 0
        },
        {
            question: "Który z tych pierwiastków jest cieczą w temperaturze pokojowej?",
            answers: ["Rtęć", "Żelazo", "Tlen", "Wapń"],
            correct: 0
        },
        {
            question: "Jak nazywa się najstarszy uniwersytet w Polsce?",
            answers: ["Uniwersytet Warszawski", "Uniwersytet Jagielloński", "Politechnika Warszawska", "UMCS"],
            correct: 1
        },
    ],    2000: [
        {
            question: "Ile wynosi pierwiastek kwadratowy z 144?",
            answers: ["10", "12", "14", "16"],
            correct: 1
        },
        {
            question: "W którym roku zakończyła się II wojna światowa?",
            answers: ["1944", "1945", "1946", "1947"],
            correct: 1
        },
        {
            question: "Kto wynalazł żarówkę?",
            answers: ["Nikola Tesla", "Thomas Edison", "Alexander Bell", "Benjamin Franklin"],
            correct: 1
        },
        {
            question: "Który z tych instrumentów jest instrumentem strunowym?",
            answers: ["Flet", "Gitara", "Bębny", "Trąbka"],
            correct: 1
        },
        {
            question: "Ile lat ma dekada?",
            answers: ["5", "10", "15", "20"],
            correct: 1
        },
        {
            question: "Który kraj ma kształt buta?",
            answers: ["Francja", "Hiszpania", "Niemcy", "Włochy"],
            correct: 3
        },
        {
            question: "Który z tych pierwiastków jest metalem?",
            answers: ["Tlen", "Wapń", "Azot", "Hel"],
            correct: 1
        },
        {
            question: "Jak nazywa się stolica województwa małopolskiego?",
            answers: ["Kraków", "Katowice", "Rzeszów", "Lublin"],
            correct: 0
        },
        {
            question: "Który z tych wynalazców opatentował żarówkę?",
            answers: ["Edison", "Tesla", "Bell", "Franklin"],
            correct: 0
        },
        {
            question: "Jak nazywa się największa pustynia na świecie?",
            answers: ["Gobi", "Sahara", "Kalahari", "Atakama"],
            correct: 1
        },
    ],    5000: [
        {
            question: "Które miasto jest stolicą Polski?",
            answers: ["Kraków", "Poznań", "Gdańsk", "Warszawa"],
            correct: 3
        },
        {
            question: "Kto skomponował 'Marsz weselny'?",
            answers: ["Mozart", "Chopin", "Mendelssohn", "Bach"],
            correct: 2
        },
        {
            question: "W którym kraju znajduje się Wielki Kanion?",
            answers: ["Kanada", "Meksyk", "USA", "Brazylia"],
            correct: 2
        },
        {
            question: "W którym roku Kolumb odkrył Amerykę?",
            answers: ["1490", "1492", "1494", "1496"],
            correct: 1
        },
        {
            question: "Która planeta jest największa w Układzie Słonecznym?",
            answers: ["Saturn", "Jowisz", "Neptun", "Uran"],
            correct: 1
        },
        {
            question: "Który z tych krajów leży w Ameryce Południowej?",
            answers: ["Hiszpania", "Brazylia", "Portugalia", "Włochy"],
            correct: 1
        },
        {
            question: "Jak nazywa się najdłuższa rzeka w Polsce?",
            answers: ["Odra", "Wisła", "Warta", "Bug"],
            correct: 1
        },
        {
            question: "Który z tych malarzy był impresjonistą?",
            answers: ["Monet", "Picasso", "Van Gogh", "Dali"],
            correct: 0
        },
        {
            question: "Jak nazywa się największa planeta Układu Słonecznego?",
            answers: ["Saturn", "Jowisz", "Mars", "Ziemia"],
            correct: 1
        },
    ],10000: [
        {
            question: "Który z tych pierwiastków jest gazem szlachetnym?",
            answers: ["Tlen", "Neon", "Wodór", "Chlor"],
            correct: 1
        },
        {
            question: "Kto napisał 'Krzyżaków'?",
            answers: ["Adam Mickiewicz", "Henryk Sienkiewicz", "Bolesław Prus", "Stefan Żeromski"],
            correct: 1
        },
        {
            question: "Ile chromosomów ma człowiek?",
            answers: ["44", "46", "48", "50"],
            correct: 1
        },
        {
            question: "W którym roku upadł Mur Berliński?",
            answers: ["1987", "1989", "1991", "1993"],
            correct: 1
        }
    ],    20000: [
        {
            question: "Kto namalował 'Mona Lisę'?",
            answers: ["Leonardo da Vinci", "Michelangelo", "Rafael", "Van Gogh"],
            correct: 0
        },
        {
            question: "Ile wynosi prędkość światła w próżni?",
            answers: ["299 792 458 m/s", "300 000 000 m/s", "280 000 000 m/s", "320 000 000 m/s"],
            correct: 0
        },
        {
            question: "Który kompozytor napisał 'Symfonię z Nowego Świata'?",
            answers: ["Mozart", "Beethoven", "Dvořák", "Brahms"],
            correct: 2
        },
        {
            question: "Jaka jest najmniejsza jednostka materii?",
            answers: ["Molekuła", "Atom", "Proton", "Kwark"],
            correct: 3
        }
    ],    40000: [
        {
            question: "W którym roku Polska wstąpiła do Unii Europejskiej?",
            answers: ["2002", "2003", "2004", "2005"],
            correct: 2
        },
        {
            question: "Który organ człowieka produkuje insulinę?",
            answers: ["Wątroba", "Trzustka", "Nerki", "Serce"],
            correct: 1
        },
        {
            question: "Kto napisał 'Dziady'?",
            answers: ["Adam Mickiewicz", "Juliusz Słowacki", "Zygmunt Krasiński", "Cyprian Norwid"],
            correct: 0
        },
        {
            question: "W którym roku odbył się pierwszy lot braci Wright?",
            answers: ["1901", "1903", "1905", "1907"],
            correct: 1
        }
    ],    75000: [
        {
            question: "Która z tych rzek jest najdłuższa?",
            answers: ["Nil", "Amazonka", "Jangcy", "Missisipi"],
            correct: 1
        },
        {
            question: "Kto odkrył penicylinę?",
            answers: ["Louis Pasteur", "Alexander Fleming", "Marie Curie", "Robert Koch"],
            correct: 1
        },
        {
            question: "Ile wynosi masa elektronu w stosunku do protonu?",
            answers: ["1/1836", "1/1000", "1/2000", "1/500"],
            correct: 0
        },
        {
            question: "W którym roku zmarł Mikołaj Kopernik?",
            answers: ["1540", "1543", "1545", "1548"],
            correct: 1
        }
    ],    125000: [
        {
            question: "Który pierwiastek chemiczny ma symbol 'Au'?",
            answers: ["Srebro", "Platyna", "Złoto", "Miedź"],
            correct: 2
        },
        {
            question: "Kto opracował teorię ewolucji?",
            answers: ["Gregor Mendel", "Charles Darwin", "Alfred Wallace", "Jean-Baptiste Lamarck"],
            correct: 1
        },
        {
            question: "W którym roku powstało NATO?",
            answers: ["1947", "1949", "1951", "1953"],
            correct: 1
        }
    ],    250000: [
        {
            question: "W którym roku odbył się pierwszy lot człowieka w kosmos?",
            answers: ["1957", "1961", "1969", "1971"],
            correct: 1
        },
        {
            question: "Kto sformułował prawa termodynamiki?",
            answers: ["Rudolf Clausius", "James Maxwell", "Ludwig Boltzmann", "Wszyscy wymienieni"],
            correct: 3
        },
        {
            question: "Ile wynosi stała Plancka?",
            answers: ["6.626 × 10⁻³⁴ J·s", "6.626 × 10⁻²⁴ J·s", "3.14 × 10⁻³⁴ J·s", "9.81 × 10⁻³⁴ J·s"],
            correct: 0
        },
        {
            question: "W którym roku Polska odzyskała niepodległość?",
            answers: ["1916", "1918", "1920", "1922"],
            correct: 1
        }
    ],    500000: [
        {
            question: "Kto jest autorem teorii względności?",
            answers: ["Isaac Newton", "Albert Einstein", "Stephen Hawking", "Niels Bohr"],
            correct: 1
        },
        {
            question: "Jakie jest pierwsze zdanie 'Pana Tadeusza'?",
            answers: ["Litwo! Ojczyzno moja!", "W małym domku", "Nad Niemnem", "W domu sędziego"],
            correct: 0
        },
        {
            question: "Ile wynosi masa protonu w jednostkach atomowych?",
            answers: ["1.007276 u", "1.008665 u", "0.999999 u", "1.000000 u"],
            correct: 0
        },
        {
            question: "W którym roku Nikola Tesla urodził się?",
            answers: ["1854", "1856", "1858", "1860"],
            correct: 1
        }
    ],    1000000: [
    {
        question: "Z gry na jakim instrumencie słynie Czesław Mozil?",
        answers: ["na kornecie", "na akordeonie", "na djembe", "na ksylofonie"],
        correct: 1
    },
    {
        question: "Ile to jest 1111 razy 1111, jeśli 1 razy 1 to 1, a 11 razy 11 to 121?",
        answers: ["12 321", "1 234 321", "111 111 111", "123 454 321"],
        correct: 1
    },
    {
        question: "Różańcową tajemnicą chwalebną nie jest:",
        answers: ["wniebowzięcie Matki Bożej", "zmartwychwstanie Jezusa", "śmierć Jezusa na krzyżu", "zesłanie Ducha Świętego"],
        correct: 2
    },
    {
        question: "Kiedy rozpoczęło się drugie tysiąclecie?",
        answers: ["1 stycznia 1000 roku", "1 stycznia 1001 roku", "1 stycznia 2000 roku", "1 stycznia 2001 roku"],
        correct: 1
    },
    {
        question: "Kto w połowie XIX wieku skomponował słynną \"Modlitwę dziewicy\", utwór grany m.in. w Japonii?",
        answers: ["Franciszek Liszt z Raiding", "Edward Grieg z Bergen", "Robert Schumann z Zwickau", "Tekla Bądarzewska z Mławy"],
        correct: 3
    },
    {
        question: "Do czworonogów, czyli tetrapodów, nie zaliczamy...",
        answers: ["Gadów", "Płazów", "Ptaków", "Owadów"],
        correct: 3
    },
    {
        question: "W jakiej cieśninie znajdują się wyspy Wczorajsza i Jutrzejsza, dzielące 21 godzin różnicy czasu?",
        answers: ["w cieśninie Bosfor", "w Cieśninie Magellana", "w cieśninie Kaletańskiej", "w Cieśninie Beringa"],
        correct: 3
    },
    {
        question: "Budowa układu pokarmowego którego ze zwierząt świadczy o tym, że jest ono drapieżnikiem?",
        answers: ["lamy", "koali", "pandy", "sarny"],
        correct: 2
    },
    {
        question: "Skąd pochodzi Conan?",
        answers: ["z Rivii", "z Oz", "z Mordoru", "z Cimmerii"],
        correct: 3
    },
    {
        question: "Odrażający drab z Kabaretu Starszych Panów dubeltówkę weźmie, wyjdzie i...:",
        answers: ["rach-ciach!", "buch, buch!", "z rur dwóch", "bum w brzuch"],
        correct: 1
    },
    {
        question: "Komiksowym \"dzieckiem\" rysownika Boba Kane’a jest:",
        answers: ["Superman", "Batman", "Spider-Man", "Captain America"],
        correct: 1
    },
    {
        question: "Kto jest mistrzem tego samego oręża, w jakim specjalizowała się mitologiczna Artemida?",
        answers: ["Zorro", "Legolas", "Don Kichot", "Longinus Podbipięta"],
        correct: 1
    },
    {
        question: "Który aktor urodził się w roku opatentowania kinematografu braci Lumière?",
        answers: ["Rudolph Valentino", "Humphrey Bogart", "Charlie Chaplin", "Fred Astaire"],
        correct: 0
    },
    {
        question: "Mowa w obronie poety Archiasza przeszła do historii jako jeden z najświetniejszych popisów retorycznych:",
        answers: ["Izokratesa", "Cycerona", "Demostenesa", "Kwintyliana"],
        correct: 1
    },
    {
        question: "Likier maraskino produkuje się z maraski, czyli odmiany:",
        answers: ["wiśni", "jabłoni", "figi", "gruszy"],
        correct: 0
    },
    {
        question: "Który utwór Juliusza Słowackiego napisany jest prozą?",
        answers: ["Godzina myśli", "W Szwajcarii", "Anhelli", "Arab"],
        correct: 2
    },
    {
        question: "„Wątróbka z cebulką (...) jest zakąską doskonałą…” – to Lem i...",
        answers: ["Solaris", "Opowieści o pilocie Pirxie", "Dyktanda czyli…", "Szpital Przemienienia"],
        correct: 2
    },
    {
        question: "Co według Leszka Kołakowskiego jest sklepieniem domu, w którym duch ludzki mieszka?",
        answers: ["Rozum", "Bóg", "Miłość", "Czas"],
        correct: 3
    },
    {
        question: "Który instrument stroi muzyk?",
        answers: ["Tamburyn", "Kocioł", "Okarynę", "Czynele"],
        correct: 1
    },
    {
        question: "Który ssak się nie poci?",
        answers: ["owca", "koń", "człowiek", "królik"],
        correct: 3
    },
    {
        question: "Różańcową tajemnicą chwalebną nie jest?",
        answers: ["wniebowzięcie Matki Bożej", "zmartwychwstanie Jezusa", "śmierć Jezusa na krzyżu", "zesłanie Ducha Świętego"],
        correct: 2
    },
    {
        question: "Każda barwa składowa światła białego rozchodzi się w ciałach przezroczystych z inną prędkością. Największą prędkość ma:",
        answers: ["czerwona", "pomarańczowa", "żółta", "fioletowa"],
        correct: 0
    },
    {
        question: "Żydowskie Święto Świateł, zwane też Chanuką, trwa osiem dni. Ile świątecznych świec płonie ósmego dnia na specjalnym świeczniku?",
        answers: ["sześć", "siedem", "osiem", "dziesięć"],
        correct: 2
    },
    {
        question: "Niektóre z owoców dojrzewają jeszcze po zbiorach. Który zerwany niedojrzały taki pozostanie?",
        answers: ["wiśnia", "banan", "gruszka", "awokado"],
        correct: 0
    },
    {
        question: "Manat karaibski, tak jak afrykański, z kończyn ma tylko przednie. A manat azerbejdżański, tak jak turkmeński:",
        answers: ["ma i przednie, i tylne", "zastąpił rubla", "zakrywa całe ciało", "jest samotnikiem"],
        correct: 1
    },
    {
        question: "Na paryskim Łuku Triumfalnym, na pamiątkę napoleońskich zwycięstw, wyryte są nazwy pięciu polskich miast. Nie ma wśród nich:",
        answers: ["Warszawy", "Wrocławia", "Gdańska", "Ostrołęki"],
        correct: 0
    },
    {
        question: "Skąd najdalej jest do Końca Świata, znanego też jako Głuszyna-Podlas, w Wielkopolskiem?",
        answers: ["z Nieba w Świętokrzyskiem", "z Czyśćca w Wielkopolskiem", "z Piekła w Pomorskiem", "z Raju w Łódzkiem"],
        correct: 2
    },
    {
        question: "Miś Paddington, którego państwo Brown znaleźli na dworcu kolejowym w Londynie, pochodzi z mrocznych zakątków:",
        answers: ["Peru", "Meksyku", "Rosji", "Indii"],
        correct: 0
    }
]
};
